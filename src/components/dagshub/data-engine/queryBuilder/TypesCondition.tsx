...
export function hasUncompletedConditions(condition: AndOrMetadataInput): boolean {
    if (!!condition.or || !!condition.and) {
        ...
        return (condition.or || condition.and || [])
            .every(hasUncompletedConditions);
    } else if (!!condition.filter) {
        // If it's a simple filter, check if it's empty
        return !condition.filter.key || (!condition.filter.value && !(condition.filter?.comparator === "IS_POSITIVE_INFINITY" || condition.filter?.comparator === "IS_NEGATIVE_INFINITY" || condition.filter?.comparator === "IS_NAN" || condition.filter?.comparator === "IS_NULL"));
    }
    // Return false if it's not a filter, OR, or AND
    return false;
}

export function convertToBackandFormatAndRemoveEmptyConditions(condition: AndOrMetadataInput): AndOrMetadataInput | null {
    if (!!condition.or || !!condition.and) {
